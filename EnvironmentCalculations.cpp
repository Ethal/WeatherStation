...
/*
 modification done by ethal:
  add function FrostPoint
  improve function dewPoint
  return NAN if dewpoint < 0
  return NAN if frostpoint >= 0
 ...
 )
{
    ...
    if(!isnan(temp) && !isnan(hum))
    {
        ...
        if ((dewPoint < 0 && tempUnit == TempUnit_Celsius) || (dewPoint < 32 && tempUnit == TempUnit_Fahrenheit))
        {   
            dewPoint = NAN;
        }
    }
...
/****************************************************************/
float EnvironmentCalculations::FrostPoint
(
 float temp,
 float hum,
 TempUnit tempUnit
...
{
    // Equations courtesy from wikipedia 
    float frostPoint = NAN;
    
    if(!isnan(temp) && !isnan(hum))
    {
        float dewPoint = EnvironmentCalculations::DewPoint(temp, hum, tempUnit);
        
        if (tempUnit == TempUnit_Celsius)
        {
            float ctemp = temp + 273.15;
            float cdewPoint = dewPoint + 273.15;
                        
            frostPoint = (cdewPoint + (2671.02 / ((2954.61 / ctemp) + (2.193665 * log(ctemp)) - 13.3448)) - ctemp) - 273.15;
        }
        if (tempUnit == TempUnit_Fahrenheit)
        {
            float ctemp = ((temp - 32.0) * 5.0/9.0) + 273.15;
            float cdewPoint = ((dewPoint - 32.0) * 5.0/9.0) + 273.15;
            
            frostPoint = (cdewPoint + (2671.02 / ((2954.61 / ctemp) + (2.193665 * log(ctemp)) - 13.3448)) - ctemp) - 273.15;
            frostPoint = frostPoint * 9.0/5.0 + 32.0;
        }
        if ((frostPoint >= 0 && tempUnit == TempUnit_Celsius) || (frostPoint >= 32 && tempUnit == TempUnit_Fahrenheit))
        {   
            frostPoint = NAN;
        }
    }
